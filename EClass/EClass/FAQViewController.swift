...
class FAQViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UITabBarDelegate {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if isStudentOrTutor
        {
            if studentSelectRow[indexPath.section]
            {
                return 0
            }else
            {
        ...
        {
            if tutorSelectRow[indexPath.section]
            {
                return 0
            }else
            {
...
struct TutorFAQList {
    ...
    let long = ["누구나 수업 등록을 하실 수 있습니다.단, 수업 등록 후 관리자의 최종 승인이 있어야 홈페이지에 게시되고 신청을 받을 수 있습니다. 관리자는 등록된 수업내용, 이미지, 튜터 프로필 등의 적절성, 사 실확인을 검증한 이후 최종 승인을 해드립니다.","기본적으로 학교 근처 까페 혹은 교내 스터디룸/로비에서 수업을 진행합니다. 특별한 장소가 필요한 경우(Ex: 헬스PT, 춤, 피아노, 스포츠 등)는 EClass와 제휴된 공간을 이용하시거나, 개인연습실과 같은 개인적인 공간, 학교헬스장 등 학교시설을 이용하셔도 됩니다. 수업장소가 여의치 않은 경우 EClass센터로 우선 문의하여 주시기 바랍니다.","수강생이 신청을 하면 연결담당자가 연락을 해서 신청자분과 연결해드립니다.","‘첫 달’에 한해서 첫 회 수업료(첫 1시간 수업료)의 연결수수료가 있습니다. 연결수수료는 수강생이 튜터와 연결되기 전 EClass 측으로 선 결제하게 됩니다.","첫 수업 이후, 향후 수업료(1달 기준 수업료에서 첫 회 1시간 수업료를 제외한 요금)는 수강생에게 직접 전달받으시면 됩니다."]
}
